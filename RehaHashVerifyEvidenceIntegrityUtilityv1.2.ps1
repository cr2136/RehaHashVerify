...
$grpFiles.Text = "Files to Verify (drag-and-drop ZIP and/or Keychain)"
...
$worker.Add_RunWorkerCompleted({
    ...
    if ($zipSection -and $kcSection) {
        $summary += "Processed: ZIP + Keychain"
    } elseif ($zipSection) {
        $summary += "Processed: ZIP only"
    } elseif ($kcSection) {
        $summary += "Processed: Keychain only"
    }
    $summary = @()
...
$btnRun.Add_Click({
    try {
        ...
        if (-not [string]::IsNullOrWhiteSpace($kcPath)) {
        ...
        if ([string]::IsNullOrWhiteSpace($kcPath)) {
            $txtKCSHA.Text = ""
            $txtKCMD5.Text = ""
        }
